...
class HtmlToMarkdownCustomizedSample {
    static class CustomLinkResolver implements HtmlLinkResolver {
        ...
        @Override
        public ResolvedLink resolveLink(org.jsoup.nodes.Node node, HtmlNodeConverterContext htmlNodeConverterContext, ResolvedLink resolvedLink) {
            // convert all links from http:// to https://
            if (resolvedLink.getUrl().startsWith("http:")) {
                return resolvedLink.withUrl("https:" + resolvedLink.getUrl().substring("http:".length()));
            }
            return resolvedLink;
        }
